...
do
    if [ "$first_normal_sample" == '' ]
    then
    first_normal_sample="$line"
    continue
    fi

    if [ "$second_normal_sample" == '' ]
    then
    second_normal_sample="$line"
    continue
    fi
    ...
    echo -e "\e[2mFirst  Normal Sample:\e[0m \e[95m$first_normal_sample\e[0m"
    echo -e "\e[2mSecond Normal Sample:\e[0m \e[95m$second_normal_sample\e[0m"
...
#    ./pipeline-example-tumor_normal.sh "$first_tumor_sample" "$second_tumor_sample" "$first_normal_sample" "$second_normal_sample" "$sample"
     ./Dog_TN_081819.sh "$first_normal_sample" "$second_normal_sample" "$first_tumor_sample" "$second_tumor_sample" "$sample"
